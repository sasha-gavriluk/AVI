...
class FinancialContext:
    ...
    def generate_financial_briefing(
        ...
        # 1. Розгалуження за режимами торгівлі
        if self.trade_mode == 'binary_options':
            # Для БО нам не потрібні SL/TP рівні
            payout = self.settings.get("bo_payout_rate_pct", 82.0) # дефолтний відсоток виплати брокера
            
            position_info = self.advisor.calculate_bo_stake(
                capital=self.total_capital,
                risk_per_trade_pct=self.risk_pct,
                payout_rate_pct=payout
            )
            sl_tp_info = {
                "estimated_duration_bars": signal_duration if signal_duration else 1 # час експірації в барах
            }
        
        elif self.trade_mode == 'spot':
            sl_tp_info = self._calculate_sl_tp(current_price, direction, atr_value, signal_duration, signal_data)
            if "error" in sl_tp_info: return {"status": "error", "reason": sl_tp_info["error"]}
            
            position_info = self.advisor.calculate_position_size(
                capital=self.total_capital, risk_per_trade_pct=self.risk_pct,
                entry_price=current_price, stop_loss_price=sl_tp_info['stop_loss_price']
            )
            ...
            liquidation_info = self.advisor.calculate_liquidation_price(
                entry_price=current_price, leverage=self.leverage, direction=direction
            )
            position_info.update(liquidation_info)
        else:
        ...
        # 2. Оцінка ризику для портфеля (працює однаково для всіх, рахує risk_per_trade_usd)
        # Але для БО додаємо захист: risk_per_trade_usd дорівнює самій ставці stake_usd
        if self.trade_mode == 'binary_options':
            position_info["risk_per_trade_usd"] = position_info["stake_usd"]
            
        portfolio_risk_info = self._evaluate_portfolio_risk(position_info)

        # 3. Формування фінального брифінгу
        return {
            "status": "ok",
    ...
    def _calculate_sl_tp(self, current_price, direction, atr_value, signal_duration, signal_data=None):
        ...
        if signal_data:
            sl_tp_params["support_price"] = signal_data.get("support_price")
            sl_tp_params["resistance_price"] = signal_data.get("resistance_price")
