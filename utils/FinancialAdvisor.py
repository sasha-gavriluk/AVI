...
class FinancialAdvisor:
    ...
    def calculate_sl_tp_levels(
        ...
        sl_price = 0.0
        atr_pad = (atr_value * 0.2) if (atr_value and atr_value > 0) else (entry_price * 0.002)
        
        # 1. Розрахунок класичного (резервного) Stop Loss
        fallback_sl_price = 0.0
        if stop_loss_pct:
            fallback_sl_price = entry_price * (1 - stop_loss_pct / 100.0) if direction == 'buy' else entry_price * (1 + stop_loss_pct / 100.0)
            ...
            fallback_sl_price = entry_price - move if direction == 'buy' else entry_price + move
        else:
            return {"error": "Необхідно надати stop_loss_pct або atr_value."}

        # 2. Інтеграція розумного Stop Loss по рівнях S&R
        if direction == 'buy' and support_price and not np.isnan(support_price) and support_price < entry_price:
            dist_to_sup = entry_price - support_price
            dist_to_fallback = entry_price - fallback_sl_price
            # Якщо підтримка не далі ніж вдвічі від класичного стопа, ховаємо стоп за неї
            if dist_to_sup <= dist_to_fallback * 2.0:
                sl_price = support_price - atr_pad
            else:
                sl_price = fallback_sl_price
        elif direction == 'sell' and resistance_price and not np.isnan(resistance_price) and resistance_price > entry_price:
            dist_to_res = resistance_price - entry_price
            dist_to_fallback = fallback_sl_price - entry_price
            # Якщо опір не далі ніж вдвічі від класичного стопа, ховаємо стоп за нього
            if dist_to_res <= dist_to_fallback * 2.0:
                sl_price = resistance_price + atr_pad
            else:
                sl_price = fallback_sl_price
        else:
            sl_price = fallback_sl_price

        # 3. Розрахунок Take Profit
        risk_per_unit = abs(entry_price - sl_price)
        fallback_tp_price = entry_price + (risk_per_unit * risk_reward_ratio) if direction == 'buy' else entry_price - (risk_per_unit * risk_reward_ratio)
        tp_price = fallback_tp_price

        if direction == 'buy' and resistance_price and not np.isnan(resistance_price) and resistance_price > entry_price:
            smart_tp = resistance_price - atr_pad
            # Якщо до опору хороша відстань (R:R хоча б >= 1.0), ставимо туди. Інакше беремо класичний R:R
            if (smart_tp - entry_price) >= risk_per_unit * 1.0:
                tp_price = smart_tp
        elif direction == 'sell' and support_price and not np.isnan(support_price) and support_price < entry_price:
            smart_tp = support_price + atr_pad
            if (entry_price - smart_tp) >= risk_per_unit * 1.0:
                tp_price = smart_tp
    ...
    def calculate_bo_stake(
        ...
        stake_usd = capital * (risk_per_trade_pct / 100.0)
        
        # Мінімальна сума угоди для Бінарних Опціонів - 1$
        if stake_usd < 1.0:
            if capital >= 1.0:
                stake_usd = 1.0
            else:
                return {"error": "Недостатньо коштів для мінімальної ставки (1$)."}
                
        potential_profit_usd = stake_usd * (payout_rate_pct / 100.0)
        
        return {
            "position_size_usd": round(stake_usd, 2), # Для сумісності інтерфейсів назвемо position_size_usd
    ...
    def calculate_dynamic_risk_pct(
        ...
        if avg_win_to_loss_ratio <= 0:
            return {"error": "Коефіцієнт R має бути додатним."}
        ...
        if kelly_percentage <= 0:
            # Стратегія статистично збиткова при поточних W та R
            return {
                "full_kelly_pct": 0.0,
                "recommended_risk_pct": base_risk_pct * 0.5, # Мінімальний захисний ризик
                "comment": "Математичне очікування від'ємне. Знижено ставку до мінімуму."
            }

        calculated_risk = kelly_percentage * 100 * kelly_fraction
        
        # Жорстка стеля (Cap) для запобігання овер-сайзингу
        max_allowed_risk = base_risk_pct * max_multiplier
        recommended_risk = min(calculated_risk, max_allowed_risk)
        recommended_risk = max(recommended_risk, base_risk_pct) # Не нижче базової
