...
### 2026-07-03 — Фізичне видалення замороженого коду (комітом `0eeea95`)

Після коміту-снапшоту заморожений код більше не потрібен у закоментованому
вигляді — фізично видалений з робочого дерева. Повний перелік — у
`REFACTOR_LOG.md`, розділ "Фізичне видалення замороженого коду". Коротко
для Копілота:

- `utils/rules_engine/` (весь пакет), `StrategyGenerator.py`, `MarketRunner.py`
  — видалені файли, не рядкові літерали.
- `TradingCopilot.py` — з ~1300 до ~300 рядків: прибрано все, що
  стосувалося класичних стратегій (генерація/сканування/самонавчання/
  статичний аналізатор). Лишились лише 10 методів пам'яті/досвіду —
  саме той список, що вже був задокументований вище як "лишилось живим".
  Docstring класу оновлено (більше не згадує AST-аналіз стратегій, якого
  тепер немає).
- `copilot_service.py`/`CopilotLogic.py` — закоментовані блоки прибрані,
  лишились короткі "заморожено/видалено"-повідомлення в лозі рутини.
  Метод `CopilotLogic._on_trigger_signal_scan` видалено повністю (сигнал,
  що його викликав, і так відключений).
- **Свідомо не чіпалось** (не входило в заморозку Копілота, ризиковано без
  окремого рішення): `gui/logic/BacktestLogic.py` (воркери досі лениво
  імпортують видалені модулі — безпечно, бо тіло методів ніколи не
  виконується), `gui/logic/ChartLogic.py::_preload_simulation_strategy`,
  `utils/Trading/trading_service.py` (орфанний клас). Якщо колись
  повертати вкладку Бектест — ці місця доведеться переписувати заново,
  відновлення "як було" з git-історії вже недостатньо (сам rules_engine
  теж треба відновлювати).
- Стара версія будь-якого видаленого файлу — `git show 0eeea95:шлях/до/файлу`.
- **Верифікація**: headless-запуск ЦІЛОГО `main.py` (не лише
  `MainAppWindow`) через `QT_QPA_PLATFORM=offscreen`, 60 секунд — доходить
  до робочого Qt event loop без жодної помилки.

### 2026-07-03 — Стильовий рефакторинг 4 модулів Копілота (без зміни поведінки)

Продовження плану з попередньої розмови — переписати активні модулі під
стиль користувача. Ніде НЕ змінювалась бізнес-логіка/поведінка, лише
структура/дублювання. Кожен крок перевірявся `py_compile` + headless
`main.py` + окремим функціональним тестом на реальних даних.

- **`gui/logic/CopilotLogic.py`** — `StatsFetcherThread` виніс на рівень
  модуля (був визначений усередині `request_stats_async`), `PathManager`
  тепер один імпорт зверху замість трьох локальних, прибрав мертву
  перевірку `hasattr(self, 'stats_ready')` (клас-рівневий `pyqtSignal`
  завжди існує).
- **`gui/logic/copilot_service.py`** (677→624 рядків) — головна проблема:
  `settings.json` перечитувався в кожному методі окремо, а всередині циклу
  `CopilotSchedulerThread.run()` — **тричі за одну ітерацію**. Додано
  модульний helper `_load_settings()`, зведено до одного читання на
  ітерацію (`settings_data` тепер проходить через увесь цикл — гепи,
  сканування, розрахунок часу очікування). Прибрано ~10 зайвих локальних
  `import os/json/time/PathManager`. Перевірено реальним прогоном
  `CopilotSchedulerThread` (запущено, пройшло Крок 1-2 з реальним
  довантаженням прогалин через CCXT, коректно зупинено).
- **`utils/algorithms/backtesting/TradingCopilot.py`** (328→321 рядок) —
  прибрано мертві атрибути `self.trend_indicators`/`oscillators`/`patterns`
  (лишились від видаленого статичного аналізатора, ніде не читались),
  невикористану змінну `general_match` у `predict_success_chance`,
  дубльований тричі блок `kwargs = {'db_path': ...} if ... else
  {'use_default': True}` → метод `_db_kwargs()`, локальні `import numpy/
  json/math/pandas` — на модульний рівень. Перевірено функціонально на
  реальній БД (50k+ записів пам'яті): `get_memory_df`, `get_best_components`,
  `predict_success_chance`, `generate_experience_summary`,
  `record_backtest_result` — усі повертають коректні результати.
- **`gui/GuiBinder.py`** (найбільший, 788 рядків) — свідомо НЕ
  переписувався архітектурно (надто ризиковано для файлу, що зв'язує весь
  живий UI) — лише механічні, безпечні для поведінки правки: `_timestamp()`
  helper замість `__import__('datetime').datetime.now().strftime(...)`,
  повтореного 7 разів; дивний `__import__("PyQt6").QtWidgets.
  QTreeWidgetItem(...)` замінено на звичайний імпорт; прибрано 2 пари
  дубльованих локальних `Qt`/`QTreeWidgetItem` імпортів усередині
  `_bind_settings` (уже були на модульному рівні/раніше в тій самій
  функції).
