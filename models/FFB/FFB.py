...
#------------------------------
# Головний клас-класифікатор FB
#------------------------------

class FFB:
    "Класифікатор Хибних Пробоїв (False Breakout). Приймає сирий датасет, проганяє через Нейромережу і повертає його з двома новими колонками."
    ...
    def __init__(self):
        ...
        model_path = os.path.join(os.path.dirname(__file__), 'ffb_weights.pth')
        if os.path.exists(model_path):
            self.model.load_state_dict(torch.load(model_path, map_location=self.device, weights_only=True))
            self.model.eval()
        else:
            print("Попередження: Ваги FB моделі не знайдені. Запустіть тренування.")
    ...
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        "Обробляє сирий датафрейм, виконує передбачення та додає колонки FB_Bullish і FB_Bearish"
        ...
        for idx, out in zip(indices, outputs):
            bull_val = round(float(out[0]), 3)
            bear_val = round(float(out[1]), 3)
            
            # Поріг відсікання шуму (можна коригувати після тестів)
            df.iloc[idx, df.columns.get_loc('FFB_Bullish')] = bull_val if bull_val > 0.1 else 0.0
            df.iloc[idx, df.columns.get_loc('FFB_Bearish')] = bear_val if bear_val > 0.1 else 0.0
