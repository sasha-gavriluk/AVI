...
class MR:
    ...
    def __init__(self):
        ...
        model_path = os.path.join(os.path.dirname(__file__), 'mr_weights.pth')
    ...
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, out in zip(indices, outputs):
            df.iloc[idx, df.columns.get_loc('MR_Trend')] = round(float(out[0]), 3)
            df.iloc[idx, df.columns.get_loc('MR_Flat')] = round(float(out[1]), 3)
            df.iloc[idx, df.columns.get_loc('MR_Explosion')] = round(float(out[2]), 3)
